<div>
    <h1 class="text-2xl font-bold py-4 pl-4 text-green-500">TOTAL PRECIO DE VENTA Y COMPRA DE PRODUCTOS</h1>
    <div class="px-4 pt-2 md:px-6 w-full flex flex-col md:flex-row md:justify-between md:space-x-6 ">
    ...
    <div class="flex flex-col md:flex-row items-center">
        <div class="p-4 md:px-6 w-full md:w-1/2">
            @if ($productos->count())
                <div class="shadow rounded p-4 border bg-white flex-1" style="height: 30rem;">
                    <livewire:livewire-column-chart key="{{ $columnChartModelVentas->reactiveKey() }}" :column-chart-model="$columnChartModelVentas" />
                </div>
            @else
        ...
        <div class="p-4 md:px-6 w-full md:w-1/2">
            @if ($compras->count())
                <div class="shadow rounded p-4 border bg-white flex-1" style="height: 30rem;">
                    <livewire:livewire-column-chart key="{{ $columnChartModelCompras->reactiveKey() }}" :column-chart-model="$columnChartModelCompras" />
                </div>
            @else
                <div class="shadow rounded p-4 border bg-white flex-1 w-full text-center text-lg font-semibold">
