    ...
    <x-modal-simple wire:model="open_edit">
        <x-slot name="title">
            <div class="text-center text-gray-100 mt-3 font-bold">
                EDITAR PRODUCTO <span>{{ $producto->nombre }}</span>
            </div>
        </x-slot>
        ...
        <x-slot name="content">
            <div class="w-full px-4 flex items-start flex-col md:flex-row pt-4">
                <div class="w-full md:w-1/2 px-0 md:px-4">
                    <div class="w-full flex justify-center mb-10">
                        <div>
                            ...
                            <div class="w-64 h-64">
                                ...
                                @else
                                    <img class="mb-2 w-64 h-64" src="{{ Storage::url($producto->image) }}" alt="">
                                @endif
                    ...
                    <x-jet-input class="mb-2 w-full" type="text" wire:model.defer="producto.marca" />
                    <x-jet-input-error for="producto.marca" />
                    {{-- codigo --}}
                    {{-- <x-jet-label value="Insertar Codigo del Producto" />
                    <x-jet-input class="mb-2 w-full" type="text" wire:model="codigo" />
                    <x-jet-input-error for="codigo" /> --}}
                    <div>{!! DNS1D::getBarcodeSVG($codigo, 'C39', 2, 66) !!} </div>
                </div>
                <div class="w-full md:w-1/2">
                    {{-- subcategoria --}}
                    <x-jet-label value="Seleccionar Subcategoria" />
                    <select name="select" wire:model.defer="producto.subcategoria_id"
                    ...
                    <x-jet-input class="mb-2 w-full" type="text" wire:model.defer="producto.medida" />
                    <x-jet-input-error for="producto.medida" />                 
                    {{-- stock --}}
                    ...
                    <textarea rows="8" class="w-full border-2 border-gray-200 rounded-md focus:outline-none focus:shadow-none focus:border-none text-justify focus:ring-1 focus:ring-blue-200"
                        wire:model.defer="producto.descripcion"></textarea>
