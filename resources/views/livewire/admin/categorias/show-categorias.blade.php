    ...
    <x-modal-simple wire:model="open_show">
        <x-slot name="title">
            <div class="text-center text-gray-100 mt-3 font-bold">
                ...
                <span>{{ $show_categoria->nombre }}</span>
            </div>
        </x-slot>
            ...
            <div class="w-full px-4 flex items-start flex-col md:flex-row pt-4">
                <div class="w-full md:w-1/2 px-0 md:px-4">
                    <div class="w-full flex justify-center">
                        <div>
                            <div class="w-64 h-64">
                                @if ($show_categoria->image)
                                    <img class="mb-2 w-64 h-64" src="{{ Storage::url($show_categoria->image) }}"
                                        alt="">
                                @else
                    ...
                    <div class="flex flex-col items-center justify-center ">
                        ...
                        @if ($show_categoria->condicion == 'activado')
                ...
                <div class="w-full md:w-1/2">
                    {{-- nombre --}}
                    <x-jet-label value="Nombre de la CCategoria" />
                    <input type="text" value="{{ $show_categoria->nombre }}" disabled
                        class="w-full rounded-md border-2 border-gray-300">
                    {{-- slug --}}
                    <x-jet-label value="slug" />
                    <input type="text" value="{{ $show_categoria->slug }}" disabled
                        class="w-full rounded-md border-2 border-gray-300">
                    {{-- descrpcion --}}
                    ...
                    <textarea rows="5" class="w-full border-2 border-gray-200 rounded-md focus:outline-none focus:shadow-none focus:border-none text-justify focus:ring-1 focus:ring-blue-200"
                        disabled>{{ $show_categoria->descripcion }}</textarea>
                </div>
            </div>
        </x-slot>
